-- Ensure numeric outputs retain numeric type explicitly
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      COALESCE(c.total_score, 0)::int AS total_score_col,
      COALESCE(cws.score, 0)::numeric AS weekly_score_col,
...
BEGIN
  ...
  WITH crew_base AS (
    SELECT
      ...
      COALESCE(c.total_score, 0)::int AS total_score_col,
      COALESCE(cws.score, 0)::numeric AS weekly_score_col,
      COALESCE(c.max_member, 20)::int AS max_member_col,
