-- Alias weekly_score and total_score columns to avoid ambiguity
...
BEGIN
  ...
  SELECT
    ...
    weekly_score_col AS weekly_score,
    total_score_col AS total_score
  FROM ranked
  ORDER BY rnk
...
BEGIN
  ...
  SELECT
    ...
    wr.member_count_col AS member_count,
    wr.weekly_score_col AS weekly_score,
    wr.weekly_rank,
    tr.total_score_col AS total_score,
    tr.total_rank
...
BEGIN
  ...
  SELECT
    ...
    wr.logo_url_col AS logo_url,
    wr.member_count_col AS member_count,
    wr.max_member_col AS max_member,
    wr.weekly_score_col AS weekly_score,
    wr.weekly_rank,
    tr.total_score_col AS total_score,
    tr.total_rank,
    wr.introduction_col AS introduction
  FROM weekly_ranked wr
  JOIN total_ranked tr
    ON tr.crew_id_col = wr.crew_id_col
   AND tr.crew_name_col = wr.crew_name_col
