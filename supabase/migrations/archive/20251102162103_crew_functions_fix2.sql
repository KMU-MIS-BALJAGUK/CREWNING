-- Fix crew_id ambiguity in summary/overview functions
...
BEGIN
  ...
  WITH crew_base AS (
    ...
    SELECT
      ...
      max_member,
      introduction,
      ROW_NUMBER() OVER (ORDER BY weekly_score DESC, total_score DESC, crew_name) AS weekly_rank
    FROM crew_base
  ), total_ranked AS (
    SELECT
      crew_id_col,
      ...
      max_member,
      introduction,
      ROW_NUMBER() OVER (ORDER BY total_score DESC, crew_name) AS total_rank
    FROM crew_base
  )
  SELECT
    ...
    wr.introduction
  FROM weekly_ranked wr
  JOIN total_ranked tr USING (crew_id_col, crew_name, logo_url, member_count, weekly_score, total_score, max_member, introduction)
